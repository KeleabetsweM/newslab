...
export async function createArticleForJournalist({
  journalistId,
  topic
}: {
  journalistId: string;
  ...
  if (!journalist) throw new Error(`Journalist seed "${journalistId}" is missing.`);
...
`;
  ...
  return { article_id: article.id, journalist_id: journalist.id, status: 'idea' };
}
