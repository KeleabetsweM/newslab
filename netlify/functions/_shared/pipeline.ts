...
import { sendTelegramPreview } from './telegram';
...
export async function runPipelineStepForArticle(articleId: string) {
  ...
  if (currentStatus === 'idea' || currentStatus === 'revision_requested') {
    ...
    if (isMock) {
      ...
      await supabase.from('sources').insert([
        { article_id: articleId, title: 'Blaauwbank Historical Farm Directory', url: 'https://www.blaauwbankfarm.co.za', reliability_score: 0.9, notes: 'Verified opening hours and pie specialties.' },
        { article_id: articleId, title: 'Gauteng Heritage Directory', url: 'https://gautengtourism.co.za', reliability_score: 0.8, notes: 'Heritage status verified.' }
      ]);
    } else {
...
Provide the output strictly in JSON matching this schema:
{
  ...
  "prompt": "string"
    ...
    try {
      const generated = await generateFeaturedImage(promptStr, articleId);
...
Provide the output strictly in JSON matching this schema:
{
  ...
  "report": "string"
    ...
    const { data: sources } = await supabase.from('sources').select('title, url').eq('article_id', articleId);
